<?php
    ...
    $response = array();
    try {
    	if (isset($_POST["Bride_Name"]) && isset($_POST["Bride_Name"])   ) {
        	...
        	if (($row = $result->fetch(PDO::FETCH_ASSOC)) !== false) {
        		...
        		$result = $con->prepare(
    				"INSERT INTO Marriage(Groom_Name, Groom_Address, Groom_Phone_Number, Groom_Born_Place, Groom_Born_Date, Groom_Father, Groom_Mother, Bride_Name, Bride_Address, Bride_Phone_Number, Bride_Born_Place, Bride_Born_Date, Bride_Father, Bride_Mother,Blessing_Date,Address_After_Married,Phone_After_Married,BrideGroom_Approval) 
    					VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    			);
    			if ($result->execute([$Groom_Name, $Groom_Address, $Groom_Phone_Number,$Bride_Home_Number, $Groom_Born_Place, $Groom_Born_Date, $Groom_Father, $Groom_Mother, $Bride_Name, $Bride_Address, $Bride_Phone_Number,$Bride_Home_Number, $Bride_Born_Place, $Bride_Born_Date, $Bride_Father, $Bride_Mother,$Blessing_Date,$Address_After_Married,$Phone_After_Married,$BrideGroom_Approval])) {
    				$response["success"] = 1;
